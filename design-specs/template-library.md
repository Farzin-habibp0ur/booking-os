...
> Maintained by: Visual Designer Agent (P14)
> Brand reference: CLAUDE.md → Design System & UI Guidelines
...
| Element | Value |
|---------|-------|
| Primary color (Sage) | `#8AA694` (500), `#71907C` (600) |
| Accent color (Lavender) | `#9F8ECB` (500), `#8A75BD` (600) |
| Background | `#FCFCFD` |
| Dark text | `#1E293B` |
...
| Shadow | `0 12px 40px -12px rgba(0, 0, 0, 0.05)` |
...
### IG-003: Stat Card
- **Dimensions:** 1080 × 1080 px (1:1)
- **Use for:** Data points, percentage improvements, results
- **Layout:** Large number center (Playfair 72px, sage-600), context text below (Inter 18px), subtle pattern background
- **Elements:** Stat number, unit label, supporting text, source attribution (Inter 10px), logo

### IG-004: Before/After Split
- **Dimensions:** 1080 × 1080 px (1:1)
- **Use for:** Transformation stories, workflow improvements
- **Layout:** Vertical split — left (red-tinted "Before"), right (sage-tinted "After")
- **Elements:** "Before" label (top left), "After" label (top right), screenshot or illustration in each half, divider line center
...
- **Dimensions:** 1080 × 1920 px (9:16)
- **Use for:** Tutorial videos — text overlays per step
- **Layout:** Text bar at top (40% opacity dark background), step number badge, step text
- **Elements:** Step number (sage circle, 32px), step text (Inter Semibold 24px, white), progress dots at bottom
...
- **Dimensions:** 1080 × 1080 px (1:1)
- **Use for:** Professional guides, frameworks, step-by-step content
...
- **Use for:** Sharing stats, benchmarks, industry data
- **Layout:** White background, chart/graph center, title above, source below
- **Elements:** Chart (sage + lavender colors), title (Playfair 24px), axis labels (Inter 12px), source (Inter 10px)

---

## Blog Templates

### BG-001: Featured Image (Hero)
...
- **Use for:** Product screenshots embedded in blog posts
- **Layout:** Screenshot with rounded corners (16px), subtle shadow, labeled callouts
- **Elements:** Screenshot, numbered callout circles (sage), annotation text (Inter 12px)
...
| Date | Change |
|------|--------|
| 2026-03-12 | Initial creation — 13 templates across 4 platforms |
