...
Business Command Centre is a **multi-tenant SaaS platform** positioned publicly as an **AI front desk for aesthetic clinics**: it captures Instagram/WhatsApp/web leads, drafts replies for staff approval, fills cancellations, follows up consults, and proves recovered revenue. Internal package/repo names may still use `booking-os`.
...
│   │   ├── src/
│   │   │   ├── modules/        # 82 feature modules (one dir per domain)
...
│   │   ├── src/
│   │   │   ├── app/            # 96 pages (App Router) — customer-facing only, no admin/console
...
│   │   ├── src/
│   │   │   ├── app/            # 21 admin pages (App Router) — 16 core + 5 marketing, migrated from apps/web/
...
├── packages/
│   ├── db/                     # Prisma schema (89 models), 74 migrations, seed scripts
...
| AI          | Anthropic Codex API                                                                                                            | Codex-sonnet |
...
- External service errors (Codex API, WhatsApp, Stripe, Resend) should be caught, logged, and gracefully degraded — never crash the request
- `ClaudeClient` has built-in graceful degradation (returns null on failure, caller handles)

### Database (Prisma)

- Schema at `packages/db/prisma/schema.prisma` — **89 models**, 74 migrations
...
- **96 pages** in `apps/web/` + **21 admin pages** in `apps/admin/`
- Client components use `'use client'` directive

### Page Categories

> 96+ pages across public, protected, and console. See `docs/REFERENCE.md` for the full listing.
...
Single source of truth at `nav-config.ts` + `mode-config.ts`. 4 sidebar sections: Workspace / Tools / Insights / AI Front Desk. Command palette (⌘K) searches all pages.
...
- Mock external services (Codex API, WhatsApp, Stripe, Resend)
...
|----------------------------------------|---------------------------------------------------------------------------------------------------|
| New Prisma model                       | AGENTS.md model count, seed script if demo data needed                                            |
| New/changed enum value                 | AGENTS.md Key Enums section (this is the primary reference — not in Prisma schema)                |
| New API module                         | Register in `app.module.ts`, AGENTS.md module count if stated                                     |
| New page or route                      | `docs/REFERENCE.md` page categories, `nav-config.ts`, `mode-config.ts`, `locales/*.json`          |
| New environment variable               | AGENTS.md env vars table, `.env.example` with comment                                             |
| Design system change (colors/tokens)   | `design-tokens.ts`, AGENTS.md Design System section, `DESIGN_DOCUMENTATION.md`                    |
| New BullMQ queue                       | AGENTS.md BullMQ section (and discuss consolidation — 8 is already a lot)                         |
| New messaging channel or provider      | `docs/CHANNEL-SETUP.md`, AGENTS.md omnichannel summary                                           |
| New agent type                         | AGENTS.md Key Enums (AgentType), `docs/PROJECT_CONTEXT.md`                                        |
| New seed script                        | `docs/REFERENCE.md` seed table                                                                    |
| Auth, cookie, or CORS change           | `DEPLOY.md`, then verify with the curl command in AGENTS.md § Deployment Rules                    |
| New Socket.IO event                    | AGENTS.md Real-Time event list                                                                    |
| Deployment or infrastructure change    | `DEPLOY.md` (read it first — it documents hard-won lessons)                                       |
| New vertical pack                      | AGENTS.md Vertical Pack System, `docs/PROJECT_CONTEXT.md`, seed scripts                           |

### Rules
- **What belongs in AGENTS.md:** Rules, conventions, guardrails, frequently-referenced data (enums, env vars), and short pointers to other docs. If a coding assistant needs it almost every session, it belongs here.
...
- Never update AGENTS.md model/module/page counts with approximate numbers — count the actual items.
- When extracting content to a reference doc, always leave a pointer in AGENTS.md.
...
| AI         | `ANTHROPIC_API_KEY`                                                                     | Codex API for intent detection, replies, booking assistant                                                                   |
...
| Email      | `EMAIL_PROVIDER`, `EMAIL_API_KEY`, `EMAIL_FROM`, `SENDGRID_INBOUND_WEBHOOK_SECRET`, `PILOT_APPLICATION_NOTIFY_EMAIL` | Provider: `resend`, `sendgrid`, or `none` (default, logs only). Pilot applications notify the configured founder/admin email |
...
After completing ANY task (feature, bugfix, refactor, doc update, plan, analysis, report), run the **self-review-loop** skill (`.Codex/skills/self-review-loop/SKILL.md`). This is non-negotiable — no work is considered complete until it passes.
...
- If you changed AGENTS.md, verify it's under the target line count (see version header).
