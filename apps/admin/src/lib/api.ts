const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://api.businesscommandcentre.com/api/v1';
...
class ApiClient {
  ...
  private async request<T>(path: string, options: RequestInit = {}, isRetry = false): Promise<T> {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) {
        this.setToken(null);
        // Redirect to customer app login (admin has no local login page)
        const customerApp =
          process.env.NEXT_PUBLIC_CUSTOMER_APP_URL || 'https://businesscommandcentre.com';
        if (typeof window !== 'undefined') {
          window.location.href = `${customerApp}/login`;
        }
      }
