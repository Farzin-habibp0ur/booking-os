...
const TEMPLATE_VARS = ['{{customerName}}', '{{serviceName}}', '{{businessName}}', '{{bookingDate}}'];
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <label className="flex items-center gap-2 text-sm">
        <input
          ...
          className="rounded"
        />
        <span className="text-slate-700">Default protocol (used when no service-specific protocol exists)</span>
      </label>
      ...
      <div>
        ...
        <div className="text-xs text-slate-500 mb-3">
          Available variables: {TEMPLATE_VARS.map((v) => (
        ...
        <div className="space-y-4">
          {steps.map((step, index) => (
            ...
            >
              ...
              <div>
                <label className="block text-xs text-slate-500 mb-1">Internal Notes (optional)</label>
                <input
                  type="text"
