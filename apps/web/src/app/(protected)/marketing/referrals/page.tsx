...
export default function ReferralSettingsPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get<TopReferrer[]>('/referral/top-referrers').catch(() => []),
    ])
      .then(([cfg, s, tr]) => {
        setSettings(cfg);
        setStats(s);
  ...
  return (
    <div className="p-6 max-w-2xl">
      ...
      {/* Dashboard Stats                                              */}
      {/* ============================================================ */}
      {stats && isVerticalAllowed && (
        <div className="space-y-4 mb-6" data-testid="referral-dashboard">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {[
              { label: 'Total Referrals', value: stats.totalReferrals },
              { label: 'Completed', value: stats.completedReferrals, color: 'text-sage-700' },
              { label: 'Pending', value: stats.pendingReferrals, color: 'text-lavender-700' },
              { label: 'Credits Issued', value: `$${stats.totalCreditsIssued}` },
              { label: 'Credits Redeemed', value: `$${stats.totalCreditsRedeemed}` },
              {
                label: 'Conversion',
                value:
              ...
              <div key={item.label} className="rounded-2xl bg-white p-4 text-center shadow-soft">
                <p className={cn('font-serif text-2xl font-bold', item.color || 'text-slate-800')}>
                  {item.value}
                </p>
          ...
          {topReferrers.length > 0 && (
            <div className="bg-white rounded-2xl shadow-soft p-4">
              <h3 className="text-sm font-semibold text-slate-800 mb-3">
                <TrendingUp size={14} className="inline mr-1.5 text-sage-600" />
                Top Referrers
              </h3>
                ...
                {topReferrers.map((r) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 text-xs text-slate-500">
                      <span>{r.totalReferrals} referrals</span>
                      <span className="font-medium text-sage-700">${r.totalCreditsEarned}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {stats.recentReferrals.length > 0 && (
            <div className="bg-white rounded-2xl shadow-soft p-4">
              <h3 className="text-sm font-semibold text-slate-800 mb-3">Recent Referrals</h3>
              <div className="space-y-2">
                {stats.recentReferrals.slice(0, 10).map((r) => (
                  <div
                    key={r.id}
                    className="flex items-center justify-between text-sm py-1.5 border-b border-slate-50 last:border-0"
                  >
                    <div>
                      <span className="text-slate-700">{r.referrerName}</span>
                      <span className="text-slate-400 mx-1.5">&rarr;</span>
                      <span className="text-slate-600">{r.referredName}</span>
                    </div>
                    <span
                      className={cn(
