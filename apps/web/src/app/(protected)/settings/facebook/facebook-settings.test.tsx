  ...
  useI18n: () => ({
    t: (key: string) => {
      const translations: Record<string, string> = {
        ...
        'facebook.description': 'Connect Facebook Pages for Messenger conversations',
        ...
        'facebook.messaging_window_info': 'Facebook Messenger has a 24-hour messaging window.',
        'facebook.setup_step1': 'Go to Meta Business Suite and get your Page Access Token',
        'facebook.setup_step2': "Copy your Page ID from the Page's About section",
        'facebook.setup_step3': 'Enter the credentials below',
        'facebook.no_locations': 'No locations found. Create a location first.',
      };
...
function setupDefaultMocks(
  locations: any[] = [{ id: 'loc1', name: 'Main Location', facebookConfig: null }],
) {
...
describe('FacebookSettingsPage', () => {
  ...
  it('renders page title and description', async () => {
    ...
    expect(
      screen.getByText('Connect Facebook Pages for Messenger conversations'),
    ).toBeInTheDocument();
  });
  ...
  it('shows Page Connection section with setup guidance', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('page-connection-section')).toBeInTheDocument();
    });
    expect(screen.getByText('Page Connection')).toBeInTheDocument();
    expect(
      screen.getByText('Go to Meta Business Suite and get your Page Access Token'),
    ).toBeInTheDocument();
  ...
  it('shows Ice Breakers section', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('ice-breakers-section')).toBeInTheDocument();
    });
    expect(screen.getByText('Ice Breakers')).toBeInTheDocument();
    expect(
      screen.getByText('Suggested conversation starters shown when someone opens Messenger'),
    ).toBeInTheDocument();
    expect(screen.getByTestId('add-ice-breaker-button')).toBeInTheDocument();
  });
  ...
  it('shows messaging window info card', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('messaging-window-card')).toBeInTheDocument();
    });
  ...
  it('test connection button triggers API call', async () => {
    ...
    await user.type(screen.getByTestId('page-access-token-input'), 'token-abc');
  ...
  it('can add and remove ice breakers', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('ice-breaker-question-0')).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(screen.getByTestId('ice-breaker-question-1')).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('ice-breaker-question-1')).not.toBeInTheDocument();
    });
  ...
  it('limits ice breakers to 4 maximum', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('ice-breaker-question-3')).toBeInTheDocument();
    });

    // Button should be disabled at max
    const addBtn = screen.getByTestId('add-ice-breaker-button');
    expect(addBtn).toBeDisabled();
    expect(screen.getByText('Maximum 4 ice breakers')).toBeInTheDocument();
  });
});
