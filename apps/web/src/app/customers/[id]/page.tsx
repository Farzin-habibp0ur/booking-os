...
export default function CustomerDetailPage() {
  ...
  if (loading) return <div className="p-6 flex items-center justify-center h-64"><p className="text-gray-400">Loading...</p></div>;
  if (!customer) return <div className="p-6"><p className="text-red-500">Customer not found</p></div>;
    ...
    <div className="p-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">{pack.labels.customer} since {new Date(customer.createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</p>
        </div>
        <button onClick={() => setShowBookingForm(true)} className="flex items-center gap-1 bg-blue-600 text-white px-3 py-2 rounded-md text-sm hover:bg-blue-700">
          <Plus size={14} /> New {pack.labels.booking}
        </button>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border rounded-lg p-4 space-y-3">
            <h3 className="text-sm font-semibold text-gray-500 uppercase">Contact</h3>
          ...
          {(pack.customerFields.length > 0 || (customer.customFields && Object.keys(customer.customFields).length > 0)) && (
            <div className="bg-white border rounded-lg p-4">
              <h3 className="text-sm font-semibold text-gray-500 uppercase mb-2">
                {pack.name !== 'general' ? `${pack.labels.customer} Details` : 'Custom Fields'}
              </h3>
          ...
          <div className="bg-white border rounded-lg">
            ...
            {tab === 'bookings' && (
              <div className="p-4">
                ...
                {/* Past */}
                <div>
                  <h4 className="text-xs font-medium text-gray-500 uppercase mb-2">History</h4>
                  {pastBookings.length === 0 ? (
                    <p className="text-sm text-gray-400 py-4 text-center">No booking history</p>
                  ) : (
                    <div className="space-y-2">
                      {pastBookings.map((b) => (
                        <BookingRow key={b.id} booking={b} />
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-lg font-semibold mb-4">Edit {pack.labels.customer}</h2>
            <div className="space-y-3">
              ...
              <div>
                ...
                <input value={editTags} onChange={(e) => setEditTags(e.target.value)} placeholder="vip, allergies, new" className="w-full border rounded px-3 py-2 text-sm" />
              </div>
              {pack.customerFields.length > 0 && (
                <div className="border-t pt-3 mt-1">
                  <p className="text-xs font-semibold text-gray-500 uppercase mb-2">{pack.labels.customer} Fields</p>
                  <div className="space-y-2">
                    {pack.customerFields.map((field) => (
                      <div key={field.key}>
