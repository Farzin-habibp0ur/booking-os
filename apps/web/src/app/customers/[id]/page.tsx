...
export default function CustomerDetailPage() {
  ...
  if (loading) return <div className="p-6 flex items-center justify-center h-64"><p className="text-slate-400">{t('common.loading')}</p></div>;
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => router.push('/customers')} className="p-1.5 rounded-lg hover:bg-slate-100 transition-colors">
          <ArrowLeft size={18} />
        </button>
        <div className="flex-1">
          <h1 className="text-2xl font-serif font-semibold text-slate-900">{customer.name}</h1>
          <p className="text-sm text-slate-500">{t('customer_detail.since', { entity: pack.labels.customer, date: new Date(customer.createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) })}</p>
        </div>
        <button onClick={() => setShowBookingForm(true)} className="flex items-center gap-1 bg-sage-600 text-white px-3 py-2 rounded-xl text-sm hover:bg-sage-700 transition-colors">
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-soft p-4 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-slate-500 uppercase">{t('customer_detail.contact')}</h3>
              <button onClick={() => setEditing(true)} className="text-slate-400 hover:text-sage-600 transition-colors">
                <Pencil size={14} />
              </button>
            </div>
            {customer.name && (
          ...
          <div className="bg-white rounded-2xl shadow-soft p-4">
            <h3 className="text-sm font-semibold text-slate-500 uppercase mb-2">{t('customer_detail.tags')}</h3>
            <div className="flex flex-wrap gap-1.5">
              {(customer.tags || []).map((tag: string) => (
                <span key={tag} className="inline-flex items-center gap-1 text-xs bg-sage-50 text-sage-700 px-2 py-0.5 rounded-full">
                  {tag}
          ...
          {/* Quick Stats */}
          <div className="bg-white rounded-2xl shadow-soft p-4">
            <h3 className="text-sm font-semibold text-slate-500 uppercase mb-3">{t('customer_detail.summary')}</h3>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <p className="text-xl font-serif font-bold text-sage-600">${Math.round(totalSpent)}</p>
              ...
              <div>
                <p className={cn('text-xl font-serif font-bold', noShows > 0 ? 'text-red-600' : '')}>{noShows}</p>
          ...
          {nextBooking && (
            <div className="bg-white rounded-2xl shadow-soft p-4">
              <h3 className="text-sm font-semibold text-slate-500 uppercase mb-2">{t('customer_detail.next_appointment')}</h3>
              <div className="flex items-center gap-3">
                <div className="text-center min-w-[48px]">
                  <p className="text-xs text-slate-500">{new Date(nextBooking.startTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p>
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{nextBooking.staff?.name || t('common.unassigned')}</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="col-span-2">
          <div className="bg-white rounded-2xl shadow-soft flex flex-col" style={{ height: 'calc(100vh - 180px)' }}>
            {/* Tabs */}
            <div className="flex items-center gap-4 p-4 border-b shrink-0">
              <button onClick={() => setTab('chat')} className={cn('flex items-center gap-1.5 text-sm font-medium pb-1 transition-colors', tab === 'chat' ? 'text-lavender-600 border-b-2 border-lavender-600' : 'text-slate-500')}>
                <Sparkles size={14} /> {t('customer_detail.ai_chat_tab')}
              </button>
              <button onClick={() => setTab('bookings')} className={cn('text-sm font-medium pb-1 transition-colors', tab === 'bookings' ? 'text-sage-600 border-b-2 border-sage-600' : 'text-slate-500')}>
                {t('customer_detail.bookings_tab', { count: bookings.length })}
              </button>
              <button onClick={() => setTab('info')} className={cn('text-sm font-medium pb-1 transition-colors', tab === 'info' ? 'text-sage-600 border-b-2 border-sage-600' : 'text-slate-500')}>
                  ...
                  {chatMessages.length === 0 && (
                    <div className="flex justify-start">
                      <div className="max-w-[80%] bg-slate-100 rounded-2xl rounded-tl-sm px-4 py-2.5">
                        <p className="text-sm text-slate-700">{t('customer_detail.chat_welcome', { name: customer.name })}</p>
                      </div>
                    </div>
                  )}
            ...
            {tab === 'bookings' && (
              <div className="p-4 flex-1 overflow-y-auto">
                {upcomingBookings.length > 0 && (
                  <div className="mb-6">
                    <h4 className="text-xs font-medium text-slate-500 uppercase mb-2">{t('customer_detail.upcoming_section')}</h4>
                ...
                <div>
                  <h4 className="text-xs font-medium text-slate-500 uppercase mb-2">{t('customer_detail.history_section')}</h4>
                  {pastBookings.length === 0 ? (
                    <p className="text-sm text-slate-400 py-4 text-center">{t('customer_detail.no_booking_history')}</p>
                  ) : (
                    <div className="space-y-2">
                      {pastBookings.map((b) => (
                        <BookingRow key={b.id} booking={b} />
            ...
            {tab === 'info' && (
              <div className="p-4 space-y-4 flex-1 overflow-y-auto">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {(customer.tags || []).length > 0 ? customer.tags.map((tag: string) => (
                      <span key={tag} className="text-xs bg-sage-50 text-sage-700 px-2 py-0.5 rounded-full">{tag}</span>
                ...
                {(pack.customerFields.length > 0 || (customer.customFields && Object.keys(customer.customFields).length > 0)) && (
                  <div>
                    <p className="text-xs text-slate-500 mb-2 font-semibold uppercase">{t('customer_detail.custom_fields')}</p>
                    <div className="space-y-2">
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-soft-lg p-6 w-full max-w-md">
            <h2 className="text-lg font-serif font-semibold text-slate-900 mb-4">{t('customer_detail.edit_title', { entity: pack.labels.customer })}</h2>
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-medium mb-1">{t('customer_detail.name_label')}</label>
                <input value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">{t('customer_detail.email_label')}</label>
                <input value={editEmail} onChange={(e) => setEditEmail(e.target.value)} type="email" className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">{t('customer_detail.tags_label')}</label>
                <input value={editTags} onChange={(e) => setEditTags(e.target.value)} placeholder={t('customer_detail.tags_placeholder')} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm" />
              </div>
              {pack.customerFields.length > 0 && (
                <div className="border-t pt-3 mt-1">
                  <p className="text-xs font-semibold text-slate-500 uppercase mb-2">{t('customer_detail.fields_label', { entity: pack.labels.customer })}</p>
                  <div className="space-y-2">
                    {pack.customerFields.map((field) => (
                      <div key={field.key}>
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button onClick={() => setEditing(false)} className="px-4 py-2 border rounded-xl text-sm transition-colors">{t('common.cancel')}</button>
                <button onClick={saveEdit} className="px-4 py-2 bg-sage-600 text-white rounded-xl text-sm hover:bg-sage-700 transition-colors">{t('common.save')}</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50/60 hover:bg-slate-50 transition-colors">
      <div className="flex items-center gap-3">
        <div className="text-center min-w-[64px]">
          <p className="text-xs text-slate-500">{new Date(b.startTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p>
