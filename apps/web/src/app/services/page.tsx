  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-serif font-semibold text-slate-900">{t('services.title', { entity: pack.labels.service })}</h1>
          <p className="text-sm text-slate-500 mt-1">{t('services.active_count', { count: services.filter((s) => s.isActive !== false).length, entity: pack.labels.service.toLowerCase() })}</p>
        </div>
      ...
      {categories.map((cat) => {
        ...
        return (
          <div key={cat} className="mb-6">
            <h2 className="text-sm font-medium text-slate-500 uppercase tracking-wide mb-3">{cat}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {catServices.map((s) => (
                <div key={s.id} className={cn('bg-white rounded-2xl shadow-soft p-4 transition-opacity', s.isActive === false && 'opacity-50')}>
                  <div className="flex justify-between items-start">
                    ...
                    <button onClick={() => { setEditing(s); setShowForm(true); }} className="p-1 hover:bg-slate-100 rounded transition-colors">
                      <Pencil size={14} />
                    </button>
                  </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-soft-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <h2 className="text-lg font-serif font-semibold text-slate-900 mb-4">{initial ? t('services.form_title_edit') : t('services.form_title_add')}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1">{t('services.name_label')}</label>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t('services.name_placeholder')} required className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">{t('services.description_label')}</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder={t('services.description_placeholder')} rows={2} className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm" />
          </div>

          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-sm font-medium mb-1">{t('services.duration_label')}</label>
              <input value={durationMins} onChange={(e) => setDurationMins(e.target.value)} type="number" min="5" step="5" required className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">{t('services.price_label')}</label>
              <input value={price} onChange={(e) => setPrice(e.target.value)} type="number" step="0.01" min="0" required className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">{t('services.category_label')}</label>
              <input value={category} onChange={(e) => setCategory(e.target.value)} placeholder={t('services.category_placeholder')} required className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={bufferBefore} onChange={(e) => setBufferBefore(e.target.value)} type="number" min="0" step="5" className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm" />
              <p className="text-[10px] text-slate-400 mt-0.5">{t('services.buffer_before_hint')}</p>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">{t('services.buffer_after_label')}</label>
              <input value={bufferAfter} onChange={(e) => setBufferAfter(e.target.value)} type="number" min="0" step="5" className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm" />
          ...
          <div className="flex items-center justify-between pt-2 border-t">
            <div>
              {initial && onToggleActive && (
                <button type="button" onClick={onToggleActive} className={cn('text-sm px-3 py-1.5 rounded transition-colors', initial.isActive !== false ? 'text-red-600 hover:bg-red-50' : 'text-sage-600 hover:bg-sage-50')}>
