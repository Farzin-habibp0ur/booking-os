...
export default function DashboardPage() {
  ...
  if (!data) return <div className="p-6"><p className="text-red-500">Failed to load dashboard</p></div>;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <MetricCard
          ...
          subtitle={weekChange !== 0 ? `${weekChange > 0 ? '+' : ''}${weekChange}% vs last week` : 'Same as last week'}
        ...
        <MetricCard
          ...
          subtitle={m.newCustomersThisWeek > 0 ? `+${m.newCustomersThisWeek} this week` : undefined}
        ...
        <MetricCard
          ...
          subtitle={`${m.avgResponseTimeMins}min avg response`}
        />
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white border rounded-lg p-4">
          ...
          <p className="text-2xl font-bold mt-1">{m.avgResponseTimeMins}<span className="text-sm font-normal text-gray-400"> min</span></p>
          <p className={cn('text-xs mt-1', m.avgResponseTimeMins <= 5 ? 'text-green-600' : m.avgResponseTimeMins <= 15 ? 'text-amber-600' : 'text-red-600')}>
            {m.avgResponseTimeMins <= 5 ? 'Excellent' : m.avgResponseTimeMins <= 15 ? 'Good' : 'Needs improvement'}
          </p>
        </div>

        {/* Status Breakdown */}
        <div className="bg-white border rounded-lg p-4">
          <p className="text-sm text-gray-500 mb-2">This Week by Status</p>
          <div className="space-y-1.5">
            ...
            {(!data.statusBreakdown || data.statusBreakdown.length === 0) && (
              <p className="text-xs text-gray-400">No bookings this week</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg border">
          <div className="flex items-center justify-between p-4 border-b">
            <h2 className="font-semibold">Unassigned Conversations</h2>
                ...
                {data.unassignedConversations.map((c: any) => (
                  <div key={c.id} className="flex items-center justify-between p-2.5 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer" onClick={() => router.push('/inbox')}>
                    ...
                    <div className="text-right">
                      <span className="text-[10px] bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full">Unassigned</span>
                      {c.lastMessageAt && (
                        <p className="text-[10px] text-gray-400 mt-1">{timeAgo(new Date(c.lastMessageAt))}</p>
                      )}
                    </div>
                  </div>
                ))}
