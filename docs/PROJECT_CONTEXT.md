...
- **Campaigns** — Audience segmentation with saved segments, merge variables ({{name}}/{{service}}/{{business}}/{{date}}/{{staff}}), A/B testing, channel selection (WhatsApp/SMS/Email/Multi), throttled dispatch via notification queue, delivery tracking with conversion funnel (Sent→Delivered→Read→Booked), per-channel analytics, cost estimation, cloning, test send, unsubscribe tracking (token-based), recurring schedules (daily/weekly/biweekly/monthly)
- **Automations** — 8 built-in playbooks (vertical-aware) with rich recipe cards + custom rule builder, 9 trigger types (incl. event-based: MESSAGE_RECEIVED, CUSTOMER_CREATED, PAYMENT_RECEIVED, TESTIMONIAL_SUBMITTED, CAMPAIGN_SENT), 10 action types (incl. REQUEST_TESTIMONIAL, SEND_EMAIL, UPDATE_CUSTOMER_FIELD, WEBHOOK with 5s timeout), visual drag-and-drop workflow builder, dry-run testing, searchable/filterable activity log with CSV export, safety controls (quiet hours with timezone, frequency caps), analytics dashboard (overview/timeline/by-rule), conflict detection
...
- **C1: Testimonial Collection System** — COMPLETE (enhanced). Prisma model `Testimonial` with status (PENDING/APPROVED/REJECTED/FEATURED), source (MANUAL/REQUESTED), submissionToken (unique, for self-service), reminderSentAt. API: CRUD + approve/reject/feature (max 6 with auto-demotion) + bulk actions (approve/reject/delete) + search/sort + dashboard stats + sendRequest (generates token, emails submission link) + self-submission portal (verifyToken + submitByToken, token nullified after use) + findPublic (by slug) + automated reminder cron (5-day follow-up). Frontend: `/testimonials` admin page with i18n (50+ keys en/es), status tabs, grid cards, request modal. `/testimonials/submit/[token]` public self-service form (star rating, textarea, mobile-friendly). `/settings/testimonials` settings page (email template, auto-approve rules, display prefs, reminder config stored in `Business.packConfig.testimonials`). Customer detail page integration (testimonials section + request button). Dashboard widget (pending/featured/avgRating). Design tokens centralized in `design-tokens.ts`. Public booking page enhanced with aggregate rating banner, featured badges, full content display. TestimonialCard uses `useI18n()` for all labels.
